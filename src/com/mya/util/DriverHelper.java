...
public abstract class DriverHelper {
	...
	// Assert element present
		public Boolean isTextPresent(String locator, String value) {
			Boolean result = false;
			try {
	...
	//Select iFrame2
	public void SelectiFrameByElements(String value, int count)
	{
		WaitForElementPresent(value, 20);
		getWebDriver().switchTo().frame(getWebDriver().findElements(By.xpath(value)).get(count));
	}
	
	//Select drop down
	public void SelectiFrameByid(String value)
	{
		getWebDriver().switchTo().frame(getWebDriver().findElement(By.id(value)));
	}
	
	//Select iframe loop
	public void SelectIframeloop(String value)
	{
		String locator = "//*[contains(@class , 'messageText')]";
		int count=0 ;
		for (count=0 ; count<=10 ; count++)
		{
		getWebDriver().switchTo().frame(getWebDriver().findElements(By.xpath(value)).get(count));
		this.waitForWorkAroundTime(3000);
			if(isElementPresent(locator))
			{
				break;
			}
